...
Scans GCS buckets for security findings.
...
- Does not remediate or modify GCS buckets — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated GCS buckets monitoring — point-in-time security audit only
...
```bash
gcsspectre scan --output json | jq '.summary'
gcsspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
